...
signal resources_updated(tiles: int, data: int, reserve: int)
...
func grant_reward() -> void:
	...
	AudioManager.play_sfx("reward_granted", randf_range(0.9, 1.1))
...
func upgrade_pipes() -> bool:
	...
	if GameData.total_data >= cost:
		...
		AudioManager.play_sfx("upgrade", randf_range(0.9, 1.1))
		resources_updated.emit(GameData.current_pipe_count, GameData.total_data, GameData.data_reserve_for_auto_repairs)
...
func upgrade_rocket_phase() -> bool:
	...
	if GameData.total_data >= cost:
		...
		AudioManager.play_sfx("rocket_upgrade", randf_range(0.9, 1.1))
		resources_updated.emit(GameData.current_pipe_count, GameData.total_data, GameData.data_reserve_for_auto_repairs)
...
func reinforce_zone(zone_id: int) -> void:
	...
	if GameData.total_data >= final_cost:
		...
		AudioManager.play_sfx("zone_reinforce", randf_range(0.9, 1.1))
		resources_updated.emit(GameData.current_pipe_count, GameData.total_data, GameData.data_reserve_for_auto_repairs)
...
func spawn_hub() -> bool:
	var stage_cap = GameData.HUB_CAP_PER_STAGE.get(GameData.current_stage, GameData.MAX_HUBS)
	...
	AudioManager.play_sfx("build_hub", randf_range(0.9, 1.1))
	resources_updated.emit(GameData.current_pipe_count, GameData.total_data, GameData.data_reserve_for_auto_repairs)
...
func spawn_vent() -> bool:
	...
	AudioManager.play_sfx("build_vent", randf_range(0.9, 1.1))
	resources_updated.emit(GameData.current_pipe_count, GameData.total_data, GameData.data_reserve_for_auto_repairs)
